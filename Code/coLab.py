...
class Colab(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        #self.place_group_shot(self.main_frame)
        self.mainloop()
    ...
    def set_group_from_menu(self, menu_groupname):
        """
        ...
        if menu_groupname == "None":
            menu_groupname = self.grouplistdict[self.gOpt.get()]
    ...
    def createMainWidgets(self):
        ...
        self.main_frame=tk.Frame(self).grid()
        try:
            self.display_group_list(self.main_frame)
            self.place_logo(self.main_frame)
            self.function_buttons(self.main_frame)
        ...
        # quit button...
        self.quitButton = tk.Button(self.main_frame, text="Quit", command=self.my_quit)
        self.quitButton.grid(column=9, row=9)
    ...
    def display_group_list(self, parent):
        """
    ...
    def place_group_shot(self, parent):
        """
        ...
        try:
            ...
            self.snapshot = tk.Label(parent, image=labelimage)
            self.snapshot.image = labelimage
            self.snapshot.grid(column=column, row=row)            
        except Exception as e:
            print "Image creation exception", sys.exc_info()[0], e
            raise SystemError
        # 
        # and again - for the title
        column = 0
        titlepath = os.path.join(self.current_group.home, "Title.png")
        print "Title path:", titlepath
        try:     # same thing, for the title
            img = Image.open(titlepath)
            labelimage = ImageTk.PhotoImage(img)
            self.gtitle = tk.Label(parent, image=labelimage)
            self.gtitle.image = labelimage
            self.gtitle.grid(column=column, row=row)
        except Exception as e:
            print "Title creation exception", sys.exc_info()[0], e
            raise SystemError
            
    def place_logo(self, parent):
        ...
        labelimage = ImageTk.PhotoImage(img)
        self.logo = tk.Label(parent, image=labelimage)
        self.logo.image = labelimage
        self.logo.grid(column=column, row=row)
